...
export default function TrabajosPropios() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const res = await fetch(`http://localhost:3000/api/trabajos/finalizados/${usuario.id}?limit=10&offset=${page * 10}`)
  ...
  useEffect(() => {
    (async () => {
      const usuario = await fetchMe();
      const res = await fetch(`http://localhost:3000/api/trabajos/pendientes/${usuario.id}?limit=10&offset=${page2 * 10}`)
      const data = await res.json()
      console.log(data.data)
